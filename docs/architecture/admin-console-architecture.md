...
Backend refresh tokens, cookies, token rotation, device metadata, selective device logout, and cross-origin CORS policy remain deferred to later productization slices. The backend exposes `GET /api/v1/auth/sessions` for narrow current-user session inventory, but the admin console does not yet include a session-management screen. [ADR-0013](adr/0013-keep-admin-auth-on-bearer-session-before-cookie-rotation.md) keeps the current admin auth contract on bearer access tokens plus server-side `auth_session` validation before any refresh-token or cookie/session-rotation migration.
