...
- Admin auth currently stays on bearer access tokens plus server-side `auth_session` validation; the backend exposes narrow current-user session inventory before any refresh-token or cookie/session-rotation work.
...
- Use the current-user session inventory to evaluate whether selective logout or richer device/session-management UX is worth adding before any token transport, refresh-token, cookie/session-rotation, CSRF, or device metadata work.
