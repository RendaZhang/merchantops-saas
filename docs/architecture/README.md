...
- [admin-console-architecture.md](admin-console-architecture.md): standalone Vite admin-console module, same-origin Nginx runtime, API client boundary, token storage boundary, server-side auth-session/logout-all boundary, and deferred refresh-token lifecycle
