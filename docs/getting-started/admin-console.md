...
It does not add ticket detail, mutations, filters, pagination controls, AI actions, approval detail/actions, import upload, import replay, import AI actions, AI interaction filters, per-request AI detail, entity history drilldown, raw prompt or provider payloads, billing or ledger semantics, cross-tenant feature-flag administration, percentage rollout, batch flag editing, audit detail, AI provider configuration, or backend API changes.
...
The frontend calls `/api/v1/auth/login`, `/api/v1/context`, `/api/v1/auth/logout`, `/api/v1/auth/logout-all`, `/api/v1/tickets?page=0&size=10`, `/api/v1/import-jobs?page=0&size=10`, `/api/v1/import-jobs/{id}`, `/api/v1/import-jobs/{id}/errors?page=0&size=10`, `/api/v1/approval-requests?page=0&size=10`, `/api/v1/feature-flags`, `/api/v1/feature-flags/{key}`, and `/api/v1/ai-interactions/usage-summary` with relative `/api/...` paths. During local development, Vite proxies those calls to `http://localhost:8080`.
...
9. Select `AI Interactions` and confirm `/ai-interactions` renders current-tenant aggregate cards plus `byInteractionType`, `byStatus`, and `byPromptVersion` breakdowns from `/api/v1/ai-interactions/usage-summary`.
10. Toggle one flag, confirm the row reflects the persisted state returned by `PUT /api/v1/feature-flags/{key}`, and restore the original value before ending the smoke.
11. Sign out, log in as `ops` or `viewer`, open `/feature-flags`, and confirm the page shows `权限不足` without returning to login.
12. Refresh `/tickets`, `/feature-flags`, `/imports`, `/imports/:id` when a job id is available, `/approvals`, and `/ai-interactions` and confirm context plus route data reload without returning to login while the session is active.
13. Select `Sign out` and confirm the app returns to the login screen.
14. Log in again, select `Sign out all sessions`, and confirm the app returns to the login screen.
15. Reusing a signed-out token against `/api/v1/context` should return `401`.
...
9. Open `AI Interactions` and confirm the usage summary loads through same-origin `/api/v1/ai-interactions/usage-summary`.
10. Toggle one flag through the UI and restore the original value.
11. Refresh `/tickets`, `/feature-flags`, `/imports`, `/imports/:id` when a job id is available, `/approvals`, and `/ai-interactions` and confirm context restores while the server-side session is active.
12. Select `Sign out` and confirm the app returns to login.
13. Log in again, select `Sign out all sessions`, and confirm the app returns to login.
14. Reusing a signed-out token against `http://localhost:8081/api/v1/context` should return `401`.

## Current Session Limits

The frontend stores the JWT access token in `localStorage` for this baseline. It clears that token when it expires locally or when `/api/v1/context`, `/api/v1/tickets`, `/api/v1/import-jobs`, `/api/v1/import-jobs/{id}`, `/api/v1/import-jobs/{id}/errors`, `/api/v1/approval-requests`, `/api/v1/feature-flags`, or `/api/v1/ai-interactions/usage-summary` returns `401` or one of the current auth-ending `403` messages: `tenant is not active`, `user is not active`, or `token claims are stale, please login again`. A generic permission `403` does not clear the local session; `/feature-flags` shows an in-page `权限不足` state for ordinary permission denial.
...
`Sign out` calls `POST /api/v1/auth/logout`, revokes only the current session, clears the local token, clears the context, tickets, import-jobs, import-job detail, import-job errors, approval-requests, feature-flags, and AI interaction usage-summary query caches, and returns to login even if the logout request fails or the token is already invalid.

`Sign out all sessions` calls `POST /api/v1/auth/logout-all`. On success, the backend revokes every active session for the same current tenant/user, then the frontend clears the same local token plus context, tickets, import-jobs, import-job detail, import-job errors, approval-requests, feature-flags, and AI interaction usage-summary query caches. Other users and other tenants are unaffected. If the request fails, the frontend still clears the local token and returns to login, but it warns that other sessions may still be active.
