...
- Full workflow data screens beyond the current Sessions inventory, Tickets queue/detail activity with plain internal comment composer, Imports queue/detail diagnostics, Approvals queue/detail review controls, Feature Flags, and AI Interactions admin screens
...
- The Sessions screen renders the current-user `GET /api/v1/auth/sessions` inventory without raw `sid` or device metadata.
