...
MerchantOps SaaS currently sits on a completed Week 1-4 baseline and an active Week 5 import-delivery stream. The public surface now covers tenant-scoped user management, ticket workflow, audit/approval, and import jobs, while AI-specific workflow features remain planned rather than public.
...
- Week 5 Slice C is now underway with paged import error reporting and detail-level `errorCodeCounts`.
...
- Import jobs: tenant-scoped create/list/detail/error-page flow with `USER_CSV` processing, filtered queue reads, quoted CSV record parsing, `errorCodeCounts`, and row-level item errors.
...
- Tenant-scoped import queue reporting with stable ordering, paged failure-item reads, and `requestedBy` / `hasFailures` / `errorCodeCounts` hints.
- Focused automated coverage for auth, user management, ticket workflow, import jobs, audit, and approval behavior.

## Active Week 5 Work

- Keep the landed `USER_CSV` schema, filtered queue/error read surfaces, and examples aligned across Swagger, `api-demo.http`, reference docs, and runbooks.
- Use the new `/api/v1/import-jobs/{id}/errors` surface and detail `errorCodeCounts` as the Week 5 reporting baseline before adding replay semantics.
- Continue Week 5 with the next narrow import follow-up such as retry/replay semantics or chunk controls without pulling Week 6 AI scope forward too early.
...
- Import job list now supports `status`, `importType`, `requestedBy`, and `hasFailuresOnly`; `/errors` supports `page`, `size`, and `errorCode`; retry/replay and chunk controls are still pending.
- Import job detail now exposes `errorCodeCounts` and still returns full `itemErrors`; a leaner large-job detail payload and broader import reporting/export flows are still deferred.
...
- Focused automated coverage exists for the current auth + user-management + ticket + import + audit + approval path, including paged import error reporting, but Swagger rendering, real infra health, and modules outside that path still need manual verification.
