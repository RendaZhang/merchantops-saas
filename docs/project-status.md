...
MerchantOps SaaS now sits on a completed Week 1-6 baseline with Week 7 AI Copilot for Import and Data Quality active. The public surface covers tenant-scoped user management, ticket workflow, audit/approval, import jobs, read-only ticket AI summary, triage, internal reply-draft, interaction-history endpoints, and the first read-only import AI error-summary endpoint that proves the project has moved from async workflow infrastructure into a credible AI-assisted business path.
...
- The current active phase remains Week 7 AI Copilot For Import And Data Quality, with mapping suggestion and fix recommendation still queued behind the new error-summary baseline.
...
- AI-assisted import read path: `POST /api/v1/import-jobs/{id}/ai-error-summary` for a suggestion-only summary built from current-tenant import detail, `errorCodeCounts`, and the first sanitized failed-row window without forwarding raw CSV payload values to the provider.
...
- Import jobs: tenant-scoped create/list/detail/ai-error-summary/replay/replay-file/selective-replay/edited-replay/error-page flow with `USER_CSV` processing, filtered queue reads, quoted CSV record parsing, `errorCodeCounts`, row-level item errors, replay-derived job lineage, scope-only replay audit metadata, and the current Week 5 plus Week 7 Slice A reporting/guidance surface.
...
- The current public AI slices are `GET /api/v1/tickets/{id}/ai-interactions`, `POST /api/v1/tickets/{id}/ai-summary`, `POST /api/v1/tickets/{id}/ai-triage`, `POST /api/v1/tickets/{id}/ai-reply-draft`, and `POST /api/v1/import-jobs/{id}/ai-error-summary`.
- All five slices are read-only. The four `POST` endpoints remain suggestion-only: they do not change ticket status, write comments, mutate import jobs, mutate import error rows, or trigger approvals or replay flows, and the `GET` history endpoint only exposes narrowed stored metadata.
...
- The import AI context is limited to current-tenant import detail, `errorCodeCounts`, and the first 20 failed rows after local structural sanitization; raw `itemErrors.rawPayload` values, including `USER_CSV.password`, are not forwarded to the provider.
...
- The import error-summary slice persists `ai_interaction_record` rows for traceability but intentionally does not add a public import AI history endpoint yet.
- The current provider ownership model is instance-level configuration rather than tenant BYOK.
- The completed Week 6 ticket AI baseline should now stay stable while Week 7 continues with import mapping suggestion and fix recommendation; automatic write-back remains out of scope.
...
- Public AI now includes ticket interaction history, ticket summary/triage/internal reply draft, and import AI error summary; import mapping suggestion, fix recommendation, public import AI history, and agentic write paths are still pending.
