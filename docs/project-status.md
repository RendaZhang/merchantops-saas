...
- The current Week 9 governance baseline now includes an executable six-workflow inventory plus one shared comparator pass for golden, failure, and policy datasets; broader usage/cost governance still comes after that baseline rather than before it.
...
- Fresh default regression on 2026-04-05 passed with `BUILD SUCCESS` and `Tests run: 367, Failures: 0, Errors: 0, Skipped: 1`.
