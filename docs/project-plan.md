...
- Week 1-4 已构成当前稳定基线，Week 5 当前公开基线已经覆盖导入提交、顺序 chunk 执行、处理中计数推进、错误分页、失败行 replay 与按精确 errorCode 的 selective replay，后续计划应建立在用户管理、ticket workflow 和 audit/approval 治理骨架之上。
...
- 上传文件并创建 job
- MQ 投递与 worker 消费
- 失败行 replay、按精确 errorCode selective replay / derived-job lineage
...
- 后台任务独立推进
- 可查看进度、失败原因和错误报告
- 可基于失败行创建新的派生 replay job，并支持按精确 errorCode 做 selective replay，而不重置原 job
