...
Latest Productization Baseline frontend workspace result on 2026-05-11:
...
- mocked browser smoke against the Vite dev server verified `/approvals` to `/approvals/:id`, pending approve and reject inline confirmation, resolved approval requests without review buttons, long and invalid payload rendering, invalid id, 403/404, mutation `400`, auth-ending stale-claims redirect, and mobile body-width containment with a long payload
...
Then follow [deployment-runtime-smoke-test.md](deployment-runtime-smoke-test.md) for health, admin page load, login, context, Tickets, Feature Flags, Imports, Import Detail when a job id is available, Approvals, Approval Detail when an approval id is available, AI Interactions, sign-out, and old-token `401` checks.
