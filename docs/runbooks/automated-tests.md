...
- `.\mvnw.cmd -pl merchantops-api -am "-Dtest=AuthSecurityIntegrationTest,OpenApiDocumentationIntegrationTest,JpaAuthSessionAdapterTest" "-Dsurefire.failIfNoSpecifiedTests=false" test` completed successfully
- `merchantops-infra` module summary: `Tests run: 6, Failures: 0, Errors: 0, Skipped: 0`
- `merchantops-api` module summary: `Tests run: 48, Failures: 0, Errors: 0, Skipped: 0`

Latest focused Productization Baseline Slice G-C1 auth-session inventory result on 2026-05-19:

- `.\mvnw.cmd -pl merchantops-api -am "-Dtest=AuthSecurityIntegrationTest,JpaAuthSessionAdapterTest,OpenApiDocumentationIntegrationTest" "-Dsurefire.failIfNoSpecifiedTests=false" test` completed successfully
- `Tests run: 46, Failures: 0, Errors: 0, Skipped: 0`

Latest focused Productization Baseline Slice I2 ticket child table tenant-linkage result on 2026-05-18:

- `.\mvnw.cmd -pl merchantops-api -am "-Dtest=TicketWorkflowIntegrationTest,TicketChildTableTenantLinkageMigrationTest" "-Dsurefire.failIfNoSpecifiedTests=false" test` completed successfully
...
Latest post-`v0.8.0-beta` Workflow Recovery Slice B frontend workspace result on 2026-06-01:
...
- a static built-admin mock server served `/approvals` with mocked context, login, and approval-request list responses
- the initial route load called `GET /api/v1/approval-requests?page=0&size=10`
- status, action-type, requester, and combined filters sent only normalized non-empty query parameters, including trimming `requestedBy`
- `Clear` returned to the unfiltered `page=0&size=10` request
- invalid `requestedBy` showed inline validation and made no approval-request call
- generic `403 permission denied` stayed inline on `/approvals`
- a `401` approval-request response cleared local auth state and redirected to `/login`
- mobile viewport `390x844` kept body-level horizontal overflow contained while the approval table remained inside its own scroll container

Latest mocked admin selective replay proposal browser smoke result on 2026-05-27:

- a static built-admin mock server served `/imports/7001` with mocked import detail, error-page, context, and selective replay proposal responses
- selecting an error code and submitting optional reviewer context created a proposal and rendered the returned `Approval request #9401` link to `/approvals/9401`
- submitting with no selected error code showed inline validation and made no proposal POST
- a generic `403 permission denied` proposal response stayed inline as a `USER_WRITE` permission message without leaving `/imports/7001`
- a `401` proposal response cleared local auth state and redirected to `/login`
- mobile viewport `390x844` kept the proposal panel within the viewport with no page-level horizontal overflow
...
Then follow [deployment-runtime-smoke-test.md](deployment-runtime-smoke-test.md) for health, admin page load, login, context, Sessions, Tickets, Ticket Detail when a ticket id is available, Feature Flags, Imports, Import Detail when a job id is available, Approvals, Approval Detail when an approval id is available, AI Interactions, sign-out, and old-token `401` checks.
...
- `AuthSecurityIntegrationTest`
  ...
  - server-side `auth_session` creation, future expiry, DB `expires_at` alignment with JWT expiration, bad-credential no-session behavior, active-session `/api/v1/context`, current-user `GET /api/v1/auth/sessions` inventory with stable sort/current-session marker/computed status/no raw `sid` leakage, current-session logout revocation, same-token-after-logout `401`, logout-all same-user token invalidation, logout-all other-user and other-tenant preservation, logout-others current-token preservation, logout-others same-user token invalidation, logout-others other-user and other-tenant preservation, sidless signed token `401`, manually expired session `401`, retention-window cleanup of expired `ACTIVE` and old `REVOKED` rows, cleanup batch-size enforcement, recently revoked session retention, old-token `401` after cleanup, and independent multi-session behavior
...
- `TicketWorkflowIntegrationTest`
  ...
  - database-level rejection when a ticket comment or operation-log row tries to reference a parent ticket from another tenant
...
- `TicketChildTableTenantLinkageMigrationTest`
  - `V20__enforce_ticket_child_table_tenant_linkage.sql` applies to a pre-V20 schema, preserves valid same-tenant comment/log parent-ticket rows, and rejects comment/log rows whose `tenant_id` does not match the parent ticket tenant
...
- `JpaAuthSessionAdapterTest`
  - persists auth-session `created_at` / `expires_at` / `revoked_at` through UTC `Instant` to `DATETIME` mapping, delegates the current-user session-list query and current-session-excluding bulk revocation query in repository order, and keeps revoke/cleanup cutoffs translated in UTC
