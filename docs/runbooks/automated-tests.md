...
Latest Productization Baseline frontend workspace result on 2026-05-10:
...
- mocked browser smoke against the Vite dev server verified `/imports/:id` desktop rendering and mobile body-width containment with long import filenames, error codes, and raw payload values
...
Then follow [deployment-runtime-smoke-test.md](deployment-runtime-smoke-test.md) for health, admin page load, login, context, Tickets, Feature Flags, Imports, Import Detail when a job id is available, Approvals, AI Interactions, sign-out, and old-token `401` checks.
