...
- [deployment-runtime-smoke-test.md](deployment-runtime-smoke-test.md): production-like admin + API smoke through Docker compose and the Nginx same-origin `/api` proxy
- [ai-live-smoke-test.md](ai-live-smoke-test.md): local `.env`-driven provider live smoke path for the current public AI surface, starting with ticket summary and expanding to import AI error summary plus mapping suggestion plus fix recommendation only after the first live pass succeeds
