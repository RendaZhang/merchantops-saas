...
- Admin console docs, smoke runbooks, roadmap/status pages, and the project showcase now describe the live Tickets queue/detail activity screen plus plain internal comment composer, Feature Flags, Imports queue/detail diagnostics, Approvals queue/detail review controls, and AI Interactions admin screens while keeping ticket creation, assignment, status transitions, filters, pagination controls, deeper import actions, bulk approval review, AI interaction detail, and backend API changes deferred.
