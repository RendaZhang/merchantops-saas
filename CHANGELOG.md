...
- Protected requests now reject sidless, revoked, expired, missing, mismatched, or cleanup-deleted auth sessions with controlled `401` responses before tenant/user/role revalidation, while refresh tokens, cookie/session rotation, and logout-all-devices remain deferred.
