...
- Access-control, user-management, migration, status, roadmap, and regression docs now distinguish the resolved `user_role` database invariant, resolved root ticket assignee/creator invariant, and resolved ticket comment/log actor invariant from the remaining child-table ticket tenant-linkage follow-up.
