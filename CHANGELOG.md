...
- Added Productization Baseline Slice H6 as an Approval Request detail and safe review-controls screen over the existing `GET /api/v1/approval-requests/{id}`, `POST /api/v1/approval-requests/{id}/approve`, and `POST /api/v1/approval-requests/{id}/reject` APIs, linking from the Approvals queue and gating pending approve/reject actions behind inline confirmation without adding backend API scope.
- Added Productization Baseline Slice H7 as a read-only Ticket detail and activity screen over the existing `GET /api/v1/tickets/{id}` API, linking from the Tickets queue and showing core ticket fields, comments, and workflow operation logs without adding backend API scope.
...
- Protected requests now reject sidless, revoked, expired, missing, mismatched, or cleanup-deleted auth sessions with controlled `401` responses before tenant/user/role revalidation, while refresh tokens, cookie/session rotation, device metadata, and per-session device logout remain deferred. The current-user session inventory exposes no raw `sid` or per-session revoke handle.
...
- Access-control, user-management, migration, status, roadmap, and regression docs now distinguish the resolved `user_role` database invariant, resolved root ticket assignee/creator invariant, resolved ticket comment/log actor invariant, and resolved ticket child-table tenant-linkage invariant.
- Admin console docs, smoke runbooks, roadmap/status pages, and the project showcase now describe the Sessions inventory with sign-out-other-sessions, the live Tickets queue/detail activity screen plus plain internal comment composer, Feature Flags, Imports queue/detail diagnostics, Approvals queue/detail review controls, and AI Interactions admin screens while keeping per-session device logout, ticket creation, assignment, status transitions, filters, pagination controls, deeper import actions, bulk approval review, and AI interaction detail deferred.
