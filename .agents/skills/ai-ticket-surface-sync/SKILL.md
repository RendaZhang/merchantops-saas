...
description: Align the repository docs for the current public ticket AI surface. Use when `POST /api/v1/tickets/{id}/ai-summary`, `POST /api/v1/tickets/{id}/ai-triage`, or `POST /api/v1/tickets/{id}/ai-reply-draft` changes, or when AI provider configuration, degraded-mode behavior, AI runbooks, AI examples, changelog wording, or phase docs must move together with the current Week 6 ticket AI implementation.
---
...
Use this skill when Week 6 ticket AI work spreads across reference docs, runbooks, examples, and milestone text at once. Keep it focused on the current public ticket AI surface rather than generic future AI work.

When the same Week 6 ticket AI change also moves phase framing or release framing, combine this skill with the narrower companion skill that owns that concern:

- pair it with [phase-status-sync](../phase-status-sync/SKILL.md) when `docs/project-status.md`, `docs/roadmap.md`, and `docs/project-plan.md` all need the same Week 6 boundary or active-slice adjustment
...
- `ai-ticket-surface-sync` owns Week 6 ticket AI reference docs, runbooks, examples, provider/runtime wording, and current AI boundary wording
...
4. Keep Week 6 AI wording precise:
...
6. Finish with an AI audit:
   ...
   - phase docs reflect the current Week 6 boundary without pretending later AI work is already public
