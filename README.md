...
- Minimal Vite/React admin console for login, current tenant context, token restoration, backend sign-out, read-only Tickets, Imports queue/detail diagnostics, and Approvals queues, a Feature Flags control screen, an AI Interactions usage-summary screen, and same-origin Nginx runtime packaging.
...
- The admin console currently includes login/context/sign-out, sign-out-all, read-only Tickets and Approvals queues, Imports queue/detail diagnostics, a Feature Flags control screen, and an AI Interactions usage-summary screen; ticket detail, ticket mutations, filters, pagination controls, approval detail/actions, AI interaction filters or per-request detail, and deeper import workflow actions remain later workflow screens.
