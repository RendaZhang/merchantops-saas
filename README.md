...
- Minimal Vite/React admin console for login, current tenant context, token restoration, backend sign-out, read-only Tickets, Imports, and Approvals queues, a Feature Flags control screen, an AI Interactions navigation placeholder, and same-origin Nginx runtime packaging.
...
- The admin console currently includes login/context/sign-out, sign-out-all, read-only Tickets, Imports, and Approvals queues, and a Feature Flags control screen; ticket detail, ticket mutations, filters, pagination controls, approval detail/actions, AI Interactions, and deeper import workflow actions remain placeholders or later workflow screens.
