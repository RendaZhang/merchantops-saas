...
- Async import jobs with create, list, detail, paged error reporting, suggestion-only AI error summary, mapping suggestion, and fix recommendation from sanitized failed-row context, failed-row replay, whole-file replay, selective replay, edited replay, queued-job recovery, and stale-processing safeguards
...
- The current public AI surface is limited to one narrowed ticket AI interaction-history read endpoint with runtime usage/cost metadata, three suggestion-only ticket generation endpoints, and three suggestion-only import AI endpoints for error summary, mapping suggestion, and fix recommendation; public import AI history, broader ticket AI flows, and any AI write-back remain pending
