...
- Current phase state: Productization Baseline release cut is complete; post-`v0.8.0-beta` Workflow Recovery and Review now has Slice A Import Selective Replay Proposal UI, Slice B Approval Queue Filters, and Slice C Import Detail Failed Rows `errorCode` Filter complete; no next implementation slice is selected yet
...
- Minimal Vite/React admin console for login, current tenant context, token restoration, backend sign-out, sign-out-all, sign-out-other-sessions, Sessions inventory, Tickets queue/detail activity with a plain internal comment composer, Imports queue/detail diagnostics with failed-row `errorCode` filtering plus selective replay proposal creation, Approvals queue filters/detail review controls, a Feature Flags control screen, an AI Interactions usage-summary screen, and same-origin Nginx runtime packaging.
...
- The admin console currently includes login/context/sign-out, sign-out-all, sign-out-other-sessions, Sessions inventory, Tickets queue/detail activity with a plain internal comment composer, Imports queue/detail diagnostics with failed-row `errorCode` filtering, Approvals queue filters/detail review controls, a Feature Flags control screen, and an AI Interactions usage-summary screen; stable public session handles, per-session revocation controls, device-aware session management, ticket creation, assignment, status transitions, additional filters outside the approval queue and Import Detail failed-row filter, pagination controls, approval bulk review/payload editing/rejection reasons, AI interaction filters or per-request detail, and deeper import workflow actions remain later workflow screens.
...
- K8s, Helm, real secret-manager integration, performance artifacts, refresh tokens, cookie/session rotation, device-aware session management beyond the current-user inventory plus logout-other-sessions action, and a broader reporting system remain outside the current baseline.
