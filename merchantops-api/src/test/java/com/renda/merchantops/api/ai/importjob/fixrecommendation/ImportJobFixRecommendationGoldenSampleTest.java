...
class ImportJobFixRecommendationGoldenSampleTest {
    ...
    @Test
    void goldenSamplesShouldKeepStableImportFixRecommendationFormatThroughRealProviderAndServicePath() throws Exception {
    ...
    private List<GoldenSample> loadSamples() throws Exception {
        try (InputStream inputStream = getClass().getResourceAsStream("/ai/import-job-fix-recommendation/golden-samples.json")) {
    ...
    private StructuredOutputAiResponse loadProviderResponse(Long importJobId) throws Exception {
        try (InputStream inputStream = getClass().getResourceAsStream("/ai/import-job-fix-recommendation/provider-response-" + importJobId + ".json")) {
            assertThat(inputStream).isNotNull();
