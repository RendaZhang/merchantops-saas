...
public class UserCsvImportProcessor {
    ...
    public void processRow(ImportJobExecutionContext context,
                           int rowNumber,
                           List<String> columns) {
        ...
        if (PasswordPolicy.hasBoundaryWhitespace(password)) {
            throw new ImportRowProcessingException("INVALID_PASSWORD", PasswordPolicy.NO_BOUNDARY_WHITESPACE_MESSAGE);
        }
        ...
        try {
            userCommandService.createUser(
                    context.tenantId(),
                    context.requestedBy(),
                    RequestIdPolicy.createImportRowRequestId(context.requestId(), rowNumber),
                    command
            );
