...
import static com.renda.merchantops.api.doc.OpenApiExamples.RESP_AUTH_SESSION_LIST;
...
public interface AuthApi {
    ...
    @ApiResponses(value = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "200",
                    description = "Current-user auth sessions",
                    content = @Content(mediaType = "application/json", examples = @ExampleObject(value = RESP_AUTH_SESSION_LIST))
            ),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "401",
                    description = "Missing, invalid, expired, or revoked auth session",
    ...
    @GetMapping("/sessions")
    ApiResponse<AuthSessionListResponse> listSessions(@Parameter(hidden = true) @AuthenticationPrincipal CurrentUser currentUser);
}
